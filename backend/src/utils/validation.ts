...
export const campaignValidator=[
    ...
    body("status").optional().isIn(["active", "inactive", "deleted"]).withMessage("Invalid status : must be one of active, inactive"),
    body("leads").isArray().withMessage("Leads must be an array of URL"),
    body("accountIDs").isArray().withMessage("Account IDs must be an array of account IDs"),
];
